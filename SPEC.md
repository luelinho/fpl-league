...
**Update, 2026-09-19 — reversed to player headshots.** The "no player photos" rule above was the owner's original call, but was explicitly reversed after seeing a reference design: pitch chips now show each player's real FPL headshot (`players.code` → `resources.premierleague.com/premierleague/photos/players/110x140/p{code}.png`, confirmed live before use) as the primary image, resized down via Pillow and cached the same way kits are (`digest/.photo_cache/`, gitignored), scoped to only players who've actually appeared in a league roster this season rather than FPL's full player universe. Club kit stays as the fallback image for any player a photo fetch fails for. Known limitation, not a bug: FPL's photo library lags behind its own team-assignment data after a real transfer — a just-transferred player's headshot can show their *previous* club's kit for days or weeks even though every other field (their `team`/`club_id`) is already correct and current. Confirmed live 2026-09-19 for three specific players (Semenyo, Rogers, Mbeumo all showed their prior club's kit in the photo despite `bootstrap-static` already reporting their current club) — this is FPL's own data being internally inconsistent, not something this pipeline can detect or correct, since there's no signal available that says "this photo is stale." Self-corrects whenever FPL updates their photo library.
