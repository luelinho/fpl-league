...
**Club badges on every pitch chip.** Roster chips show the club crest, not a player photo — deliberately: no player face photos anywhere in the dashboard. `pl_clubs` carries a `badge_code` column (FPL's own per-club image code, distinct from `club_id`), verified live against `resources.premierleague.com/premierleague/badges/50/t{badge_code}.png` (real ~6.6KB PNGs, HTTP 200, all 20 clubs). `digest.py` downloads each badge once, caches it to `digest/.badge_cache/` (gitignored), and embeds all 20 as base64 data URIs in the digest (`club_badges`, keyed by `club_id`). Every roster player object carries `club_id`; `playerChip()` shows the crest inside the jersey circle in place of the plain position-letter text, with the captain/vice armband still overlaid on top.
