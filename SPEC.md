...
A single self-contained HTML file reading `digest/season.json`. No server, no hosting, no deploy, no framework. Open it locally, or view it as an artifact.

Pages: **Home** (standings, this week's fixtures, last results, alerts) · **My Team** (roster, scores, captain and bench history, transfers, form) · **League** (full table, all matchups, H2H history) · **Analytics** (skill metrics now; luck and power rankings unlocked at GW10) · **History** (any past gameweek's standings, rosters, matchups).

Gated metrics render with their confidence label or as locked, matching the query layer. Functionality over polish, per your instruction.
